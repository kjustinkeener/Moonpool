# Assets

Images used by the top-level `README.md`.

Drop the real files here (same names the README references), then delete the
matching note below:

- `screenshot.png` - a hero screenshot of the Moonpool window: the grouped,
  searchable sidebar on the left and an app's embedded terminal on the right.
  Referenced near the top of `README.md`. Present (1341x905, shot against a set
  of fictional demo apps rather than a real machine's config, so no personal
  paths appear).
- `demo.gif` - a short (5-15s) screen recording of the core loop: pick an app
  from the sidebar, watch it launch in the embedded terminal, then stop it.
  Referenced in the README's demo slot. **Not committed yet.**

Keep these lightweight (compress the PNG, keep the GIF under a few MB) so the
README stays fast to load on GitHub.
